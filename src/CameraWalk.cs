...
namespace ZPG
{
    ...
    internal class CameraWalk : Camera
    {
        ...
        private float _currentEyeLevel = 1.8f;
        ...
        public override void Update(float dt)
        {
            ...
            if (_isJumping || _isFalling)
            {
                ...
                float terrainY = ComputeY(position.X, position.Z) + _currentEyeLevel;
            ...
            else
            {
                ...
                if (Velocity.LengthSquared > 0.0001f)
                {
                    ...
                    nextPosition.Y = ComputeY(nextPosition.X, nextPosition.Z) + _currentEyeLevel;

                    int slopeState = EvalSlope(position - new Vector3(0, _currentEyeLevel, 0), nextPosition);
                ...
                else
                {
                    ...
                    position.Y = ComputeY(position.X, position.Z) + _currentEyeLevel;
                    UpdatePosition(position);
                }
            }
        }
        ...
        public void ToggleCrouch()
        {
            ...
            if (_crouching)
            {
                _currentEyeLevel = _normalEyeLevel;
                _crouching = false;
                _speedMultiplier = 1f;
            ...
            {
                _currentEyeLevel = _crouchingEyeLevel;
                _crouching = true;
                _speedMultiplier = .4f;
                pos.Y -= _normalEyeLevel - _crouchingEyeLevel;
            }
        ...
        {
            if (_heightMap == null)
                return Position.Y - _currentEyeLevel;
